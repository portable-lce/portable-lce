...
void Tesselator::packCompactQuad() {
    ...
    // Find min x/y/z
    unsigned int minx =
        std::min<unsigned int>({m_ix[0], m_ix[1], m_ix[2], m_ix[3]});
    unsigned int miny =
        std::min<unsigned int>({m_iy[0], m_iy[1], m_iy[2], m_iy[3]});
    unsigned int minz =
        std::min<unsigned int>({m_iz[0], m_iz[1], m_iz[2], m_iz[3]});
    ...
    unsigned int minu =
        std::min<unsigned int>({m_u[0], m_u[1], m_u[2], m_u[3]});
    unsigned int minv =
        std::min<unsigned int>({m_v[0], m_v[1], m_v[2], m_v[3]});
    unsigned int maxu =
        std::max<unsigned int>({m_u[0], m_u[1], m_u[2], m_u[3]});
    unsigned int maxv =
        std::max<unsigned int>({m_v[0], m_v[1], m_v[2], m_v[3]});
