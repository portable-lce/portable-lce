...
AttributeModifier* Zombie::SPEED_MODIFIER_BABY =
    new AttributeModifier(eModifierId_MOB_ZOMBIE_BABYSPEED, 0.5f,
...
Zombie::Zombie(Level* level) : Monster(level) {
    ...
    goalSelector.addGoal(2,
                         new MeleeAttackGoal(this, eTYPE_PLAYER, 1.0, false));
    goalSelector.addGoal(3,
                         new MeleeAttackGoal(this, eTYPE_VILLAGER, 1.0, true));
    ...
    targetSelector.addGoal(
        2, new NearestAttackableTargetGoal(this, typeid(Villager), 0, false));
}
...
void Zombie::registerAttributes() {
    ...
    getAttributes()
        ->registerAttribute(SPAWN_REINFORCEMENTS_CHANCE)
        ->setBaseValue(random->nextDouble() * 0.10f);
}
                    ...
                    if (level->isUnobstructed(&reinforcement->bb) &&
                        ...
                        getAttribute(SPAWN_REINFORCEMENTS_CHANCE)
                            ->addModifier(new AttributeModifier(
                                -0.05f, AttributeModifier::OPERATION_ADDITION));
                        reinforcement->getAttribute(SPAWN_REINFORCEMENTS_CHANCE)
                            ->addModifier(new AttributeModifier(
                                -0.05f, AttributeModifier::OPERATION_ADDITION));
                        break;
                    }
...
int Zombie::getAmbientSound() { return eSoundType_MOB_ZOMBIE_AMBIENT; }

int Zombie::getHurtSound() { return eSoundType_MOB_ZOMBIE_HURT; }

int Zombie::getDeathSound() { return eSoundType_MOB_ZOMBIE_DEATH; }

int Zombie::getDeathLoot() { return Item::rotten_flesh_Id; }

void Zombie::playStepSound(int xt, int yt, int zt, int t) {
    playSound(eSoundType_MOB_ZOMBIE_STEP, 0.15f, 1);
}

MobType Zombie::getMobType() { return UNDEAD; }
...
void Zombie::killed(std::shared_ptr<LivingEntity> mob) {
                           ...
                           // need to instanceof.
    {
        ...
        level->levelEvent(nullptr, LevelEvent::SOUND_ZOMBIE_INFECTED, (int)x,
                          (int)y, (int)z, 0);
    }
}
