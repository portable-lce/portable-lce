    ...
    if (liquids) {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            int y = random->nextInt(
                random->nextInt(random->nextInt(Level::genDepth - 16) + 8) + 8);
            int z = zo + random->nextInt(16) + 8;
...
void BiomeDecorator::decorateDepthSpan(int count, Feature* feature, int y0,
                                       int y1) {
...
void BiomeDecorator::decorateDepthAverage(int count, Feature* feature, int yMid,
                                          int ySpan) {
    for (int i = 0; i < count; i++) {
        int x = xo + random->nextInt(16);
        int y =
            random->nextInt(ySpan) + random->nextInt(ySpan) + (yMid - ySpan);
        int z = zo + random->nextInt(16);
        feature->place(level, random, x, y, z);
    }
}
