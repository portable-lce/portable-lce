...
// between GL and GL ES 2 (which is most of it). It heavily depends on a bunch
// of typedefs, #defines and some utility functions that need to be set up
// correctly for the GL version being targeted. It also targets a kind of
// pseudo-GL 2.0; the platform implementation has to set up some #defines and
// perform extra initialization work if we go through extensions instead. This
// is all a bit ugly, but much easier to maintain than the original solution,
// where we just kept two almost identical versions of this code.
...
static void eat_gl_err(void) { while (glGetError() != GL_NO_ERROR); }

static void opengl_check_site(const char* site);

static void opengl_check(void) { opengl_check_site(NULL); }
...
static U32 is_pow2(S32 n) { return ((U32)n & (U32)(n - 1)) == 0; }
...
// The mac doesn't use extensions for the functions dealing with programs, and
// the non-extension versions take GLuint instead of GLhandle. The mac defines
// GDrawGLProgram to GLuint before including gdraw_gl_shared.inl to account for
// this.
...
static struct {
    S32 multisampling;  // number of samples if multisampling (always 0 if no
                        // GDRAW_MULTISAMPLING)
    ...
    // conversion from worldspace to viewspace <0,0>..<w,h> -- no translation or
    // rotation
    ...
    ProgramWithCachedVariableLocations
        fprog[GDRAW_TEXTURE__count][3][3];  // [tex0mode][additive][vformat]
    ...
    // framebuffer object used for main screen (set to non-0 to do render to
    // texture)
    ...
    // caps
    U32 has_conditional_non_power_of_two
        : 1;  // non-power-of-2 supported, but only CLAMP_TO_EDGE and can't have
              // mipmaps
...
static void make_rendertarget(GDrawHandle* t, GLuint tex, GLenum int_type,
                              GLenum ext_type, GLenum data_type, S32 w, S32 h,
                              S32 size) {
    glBindTexture(GL_TEXTURE_2D, tex);
    glTexImage2D(GL_TEXTURE_2D, 0, int_type, w, h, 0, ext_type, data_type,
                 NULL);
    make_texture(tex);
...
static void api_free_resource(GDrawHandle* r) {
    if (r->state == GDRAW_HANDLE_STATE_user_owned) return;

    if (!r->cache->is_vertex) {
        glDeleteTextures(1, &r->handle.tex.gl);
        if (r->handle.tex.gl_renderbuf &&
            r->handle.tex.gl_renderbuf != r->handle.tex.gl)
...
static void RADLINK gdraw_UnlockHandles(GDrawStats* gstats) {
    // since we're not using fences for this implementation, move all textures
    // off the active list if you're using fences, this is when the fence needs
    // to actually occur
...
extern GDrawTexture* gdraw_GLx_(WrappedTextureCreate)(S32 gl_texture_handle,
...
extern void gdraw_GLx_(WrappedTextureChange)(GDrawTexture* tex,
...
static rrbool RADLINK gdraw_MakeTextureBegin(
    ...
    GDraw_MakeTexture_ProcessingInfo* p, GDrawStats* gstats) {
    ...
    if (!t) return IGGY_RESULT_Error_GDraw;
...
static GDrawTexture* RADLINK
gdraw_MakeTextureEnd(GDraw_MakeTexture_ProcessingInfo* p, GDrawStats* stats) {
    ...
    if (format == GDRAW_TEXTURE_FORMAT_font)
        glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, width, height, 0, GL_ALPHA,
                     GL_UNSIGNED_BYTE, p->texture_data);
    else
...
static rrbool RADLINK gdraw_UpdateTextureBegin(GDrawTexture* tex,
                                               void* unique_id,
                                               GDrawStats* stats) {
...
static void RADLINK gdraw_UpdateTextureRect(GDrawTexture* tex, void* unique_id,
                                            S32 x, S32 y, S32 stride, S32 w,
                                            S32 h, U8* data,
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, w, h,
                    (format == GDRAW_TEXTURE_FORMAT_font) ? GL_ALPHA : GL_RGBA,
                    GL_UNSIGNED_BYTE, data);
    opengl_check();
}

static void RADLINK gdraw_UpdateTextureEnd(GDrawTexture* tex, void* unique_id,
                                           GDrawStats* stats) {
...
static void RADLINK gdraw_SetAntialiasTexture(S32 width, U8* rgba) {
    if (!gdraw->aa_tex) glGenTextures(1, &gdraw->aa_tex);

    make_texture(gdraw->aa_tex);
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, 1, 0, GL_RGBA,
...
static rrbool RADLINK gdraw_MakeVertexBufferBegin(
    ...
    GDraw_MakeVertexBuffer_ProcessingInfo* p, GDrawStats* gstats) {
    GLuint e;
    GDrawHandle* vb;
    opengl_check();
    ...
    if (e != GL_NO_ERROR) {
        ...
        IggyGDrawSendWarning(NULL,
                             "GDraw OpenGL vertex buffer creation failed");
        return false;
    }
    ...
    if (!gdraw->has_mapbuffer) {
        ...
        if (!p->vertex_data || !p->index_data) {
            ...
            IggyGDrawSendWarning(
                NULL, "GDraw malloc for vertex buffer temporary memory failed");
            return false;
        }
    } else {
...
static GDrawVertexBuffer* RADLINK gdraw_MakeVertexBufferEnd(
    ...
    if (!gdraw->has_mapbuffer) {
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, p->i1, p->index_data,
                     GL_STATIC_DRAW);
    ...
    if (!ok || e != GL_NO_ERROR) {
        ...
        gdraw_HandleCacheAllocateEnd(vb, p->i0 + p->i1, p->p1,
                                     GDRAW_HANDLE_STATE_locked);
...
static rrbool RADLINK gdraw_TryToLockVertexBuffer(GDrawVertexBuffer* vb,
                                                  void* unique_id,
                                                  GDrawStats* stats) {
    RR_UNUSED_VARIABLE(stats);
...
static void gdraw_ReportHandleCacheDiag(char const* label,
                                        GDrawHandleCache* cache, U32 once_bit,
                                        S32 req_w, S32 req_h) {
    GDrawHandleStateCounts counts;

    if (gdraw_rt_diag_emitted & once_bit) return;
    gdraw_rt_diag_emitted |= once_bit;

    gdraw_CountHandleStates(cache, &counts);
    IggyGDrawSendWarning(
        NULL,
        "GDraw[%s] diag: frame=%dx%d tile=%dx%d padded=%dx%d req=%dx%d "
        "depth=%d bytes_free=%d total_bytes=%d handles free=%d live=%d "
        "locked=%d dead=%d pinned=%d user=%d alloc=%d",
        label, gdraw->frametex_width, gdraw->frametex_height, gdraw->tw,
        gdraw->th, gdraw->tpw, gdraw->tph, req_w, req_h,
        (int)(gdraw->cur - gdraw->frame), cache ? cache->bytes_free : 0,
        cache ? cache->total_bytes : 0, counts.free_count, counts.live_count,
        counts.locked_count, counts.dead_count, counts.pinned_count,
        counts.user_owned_count, counts.alloc_count);
}
#else
...
static GDrawHandle* get_rendertarget_texture(int width, int height, void* owner,
                                             GDrawStats* gstats) {
    S32 size;
    GDrawHandle* t;
    opengl_check();
    ...
    glGenTextures(1, &t->handle.tex.gl);
    make_rendertarget(t, t->handle.tex.gl, GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE,
                      width, height, 4);
    t->handle.tex.w = gdraw->frametex_width;
    t->handle.tex.h = gdraw->frametex_height;
    t->handle.tex.nonpow2 =
        1;  // assume all rendertargets are non-pow2 for consistency
...
static GDrawHandle* get_color_rendertarget(GDrawStats* gstats) {
    ...
    if (gdraw->rendertargets.bytes_free < size) {
        gdraw_ReportHandleCacheDiag("rt-color-memory", &gdraw->rendertargets,
                                    GDRAW_RT_DIAG_color_memory, gdraw->tpw,
                                    gdraw->tph);
        IggyGDrawSendWarning(
            NULL, "GDraw[rt-color] exceeded available rendertarget memory");
        return NULL;
    }
    ...
    if (!t) {
        gdraw_ReportHandleCacheDiag("rt-color-handles", &gdraw->rendertargets,
                                    GDRAW_RT_DIAG_color_handles, gdraw->tpw,
                                    gdraw->tph);
        IggyGDrawSendWarning(
            NULL, "GDraw[rt-color] exceeded available rendertarget handles");
        return t;
    }
    ...
    if (gdraw->multisampling) {
        ...
        glRenderbufferStorageMultisample(GL_RENDERBUFFER, gdraw->multisampling,
                                         GL_RGBA, gdraw->frametex_width,
    ...
    gdraw_HandleCacheAllocateEnd(t, size, (void*)(UINTa)1,
                                 GDRAW_HANDLE_STATE_locked);
    gstats->nonzero_flags |= GDRAW_STATS_alloc_tex;
    ...
    if (!gdraw->stencil_depth.handle.tex.gl) {
        ...
        if (gdraw->multisampling) {
            ...
            if (gdraw->has_packed_depth_stencil) {
                ...
                glGenRenderbuffers(
                    1, &gdraw->stencil_depth.handle.tex.gl_renderbuf);
                glBindRenderbuffer(
                    GL_RENDERBUFFER,
                    gdraw->stencil_depth.handle.tex.gl_renderbuf);
            ...
            gstats->alloc_tex_bytes +=
                4 * gdraw->frametex_width * gdraw->frametex_height;
#ifdef GDRAW_MULTISAMPLING
        }
#endif
...
static void RADLINK gdraw_SetViewSizeAndWorldScale(S32 w, S32 h, F32 scalex,
                                                   F32 scaley) {
...
static void clear_with_rect(gswf_recti* region, rrbool clear_depth,
                            GDrawStats* stats);
...
static void set_render_target_state(void) {
    ...
    if (gdraw->multisampling) {
        ...
        h = gdraw->cur->color_buffer
                ? gdraw->cur->color_buffer->handle.tex.gl_renderbuf
                : 0;
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                  GL_RENDERBUFFER, h);
        h = gdraw->cur->stencil_depth ? gdraw->cur->stencil_depth->handle.tex.gl
                                      : 0;
...
#endif
        h = gdraw->cur->color_buffer ? gdraw->cur->color_buffer->handle.tex.gl
                                     : 0;
        ...
        if (gdraw->cur->stencil_depth) {
            glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                                      GL_RENDERBUFFER,
                                      gdraw->cur->stencil_depth->handle.tex.gl);
            glFramebufferRenderbuffer(
                GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
                gdraw->cur->stencil_depth->handle.tex.gl_renderbuf);
        } else {
            glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                                      GL_RENDERBUFFER, 0);
            glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT,
...
static rrbool RADLINK gdraw_TextureDrawBufferBegin(gswf_recti* region,
                                                   gdraw_texture_format format,
                                                   U32 flags, void* owner,
                                                   GDrawStats* gstats) {
    ...
    if (gdraw->tw == 0 || gdraw->th == 0) {
        gdraw_ReportHandleCacheDiag(
            "rt-empty", &gdraw->rendertargets, GDRAW_RT_DIAG_empty_request,
            region->x1 - region->x0, region->y1 - region->y0);
        IggyGDrawSendWarning(
            NULL, "GDraw[rt-stack] got a request for an empty rendertarget");
        return false;
    }

    if (n >= &gdraw->frame[MAX_RENDER_STACK_DEPTH]) {
        gdraw_ReportHandleCacheDiag(
            "rt-stack-depth", &gdraw->rendertargets, GDRAW_RT_DIAG_stack_depth,
            region->x1 - region->x0, region->y1 - region->y0);
        IggyGDrawSendWarning(NULL,
                             "GDraw[rt-stack] rendertarget nesting exceeded "
                             "MAX_RENDER_STACK_DEPTH");
        return false;
    }

    if (owner) {
        t = get_rendertarget_texture(region->x1 - region->x0,
                                     region->y1 - region->y0, owner, gstats);
        if (!t) {
            gdraw_ReportHandleCacheDiag("rt-cacheAsBitmap", gdraw->texturecache,
                                        GDRAW_RT_DIAG_cache_bitmap,
                                        region->x1 - region->x0,
                                        region->y1 - region->y0);
            IggyGDrawSendWarning(
                NULL, "GDraw[rt-cacheAsBitmap] ran out of rendertargets");
            return false;
        }
    } else {
        ...
        if (!t) {
            IggyGDrawSendWarning(NULL,
                                 "GDraw[rt-color] ran out of rendertargets");
            return false;
        }
    }
    n->color_buffer = t;
    ...
    if (region) {
        ...
        // bilinear filtering, which would mean pad = 1. however, texture
        // interpolator precision is not that high even on PC parts, and if we
        // only use 1 pixel of padding we will often get some un-filled pixels
        // "creeping in" from the sides. pad = 2 is fine on recent PC parts, but
        // not old PC parts or even fairly new mobile parts, so we play it safe
        // and use 3 pixels which so far gives good results everywhere.
        S32 pad = 3;
        ...
        // but the padding gets clamped to framebuffer coords! also transfer to
        // window space here.
        x0 = RR_MAX(xt0 - pad, 0);
        ...
        if (x1 <= x0 ||
            y1 <= y0) {  // region doesn't intersect with current tile
            --gdraw->cur;
...
static GDrawTexture* RADLINK gdraw_TextureDrawBufferEnd(GDrawStats* gstats) {
    ...
    if (m != gdraw->frame) assert(m->color_buffer != NULL);
    assert(n->color_buffer != NULL);
    ...
    if (gdraw->multisampling) {
        ...
        if (n->color_buffer->handle.tex.gl_renderbuf) {
            ...
            glBindFramebuffer(GL_READ_FRAMEBUFFER,
                              gdraw->framebuffer_copy_to_texture);
            glFramebufferRenderbuffer(
                GL_READ_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER, 0);
            glFramebufferRenderbuffer(
            ...
            glFramebufferRenderbuffer(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                      GL_RENDERBUFFER,
                                      n->color_buffer->handle.tex.gl_renderbuf);
            glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                   GL_TEXTURE_2D,
                                   n->color_buffer->handle.tex.gl, 0);
            res = glCheckFramebufferStatus(GL_DRAW_FRAMEBUFFER);
            glBlitFramebuffer(0, 0, gdraw->tpw, gdraw->tph, 0, 0, gdraw->tpw,
                              gdraw->tph, GL_COLOR_BUFFER_BIT, GL_NEAREST);
...
static void set_3d_projection(const int* vvars, const F32 world[2 * 4],
                              const F32 xform[3][4]) {
    assert(vvars[VVAR_world0] >= 0 && vvars[VVAR_world1] >= 0);
    glUniform4fv(vvars[VVAR_world0], 1, world + 0);
    glUniform4fv(vvars[VVAR_world1], 1, world + 4);

    assert(vvars[VVAR_x3d] >= 0 && vvars[VVAR_y3d] >= 0 &&
           vvars[VVAR_z3d] >= 0);
...
static int set_render_state(GDrawRenderState* r, S32 vformat,
                            const int** ovvars, GDrawPrimitive* p,
                            GDrawStats* gstats) {
    static struct gdraw_gl_blendspec {
        ...
        {GL_TRUE, GL_DST_COLOR,
         GL_ONE_MINUS_SRC_ALPHA},   // GDRAW_BLEND_multiply
    ...
    assert((vformat >= 0 && vformat < GDRAW_vformat__basic_count) ||
           vformat == GDRAW_vformat_ihud1);

    if (vformat == GDRAW_vformat_ihud1) {
        glEnable(GL_BLEND);
        glBlendFunc(GL_ONE,
                    GL_ONE_MINUS_SRC_ALPHA);  // premultiplied alpha blend mode
        prg = &gdraw->ihud[0];
    } else {
        // apply the major blend mode
        blend_mode = r->blend_mode;
        assert(blend_mode >= 0 &&
               blend_mode < sizeof(blends) / sizeof(*blends));
    ...
    if (vformat == GDRAW_vformat_ihud1) {
        ...
        if (!r->use_world_space)
            ...
            gdraw_WorldSpace(world, gdraw->world_to_pixel, depth_from_id(r->id),
                             0.0f);
    ...
    if (r->tex[0] && gdraw->has_conditional_non_power_of_two &&
        ((GDrawHandle*)r->tex[0])->handle.tex.nonpow2) {
        // only wrap mode allowed in conditional nonpow2 is clamp; this should
        // have been set when the texture was created, but to be on the safe
        // side...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    } else
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
                    r->nearest0 ? GL_NEAREST : GL_LINEAR);

    // fragment shader constants

    if (fvars[VAR_cmul] >= 0)
        glUniform4f(fvars[VAR_cmul], r->color[0], r->color[1], r->color[2],
                    r->color[3]);
    if (fvars[VAR_cadd] >= 0) {
        if (r->cxf_add)
            glUniform4f(fvars[VAR_cadd], r->cxf_add[0] / 255.0f,
                        r->cxf_add[1] / 255.0f, r->cxf_add[2] / 255.0f,
                        r->cxf_add[3] / 255.0f);
        else
...
static void set_vertex_format(S32 format, F32* vertices) {
    ...
    const void* vertex_offset_8 =
        (const void*)((size_t)vertices + (2 * sizeof(F32)));
    const void* vertex_offset_16 =
        (const void*)((size_t)vertices + (4 * sizeof(F32)));
    ...
    switch (format) {
        ...
        case GDRAW_vformat_v2aa:
            glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 16,
                                  vertex_offset_0);
            glVertexAttribPointer(1, 4, GL_SHORT, GL_FALSE, 16,
                                  vertex_offset_8);
            glEnableVertexAttribArray(0);
        ...
        case GDRAW_vformat_ihud1:
            glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 20,
                                  vertex_offset_0);
...
static void reset_vertex_format(S32 format) {
    // we don't use attrib #1 for all formats, but doesn't seem worthwhile to
    // check
    format = format;
...
static int vformat_stride[] = {2, 4, 4, 5};
...
static void RADLINK gdraw_DrawIndexedTriangles(GDrawRenderState* r,
    ...
    if (vb || p->indices) {  // regular path
        ...
        glDrawElements(GL_TRIANGLES, p->num_indices, GL_UNSIGNED_SHORT,
                       p->indices);
    } else {  // dynamic quads
        S32 pos = 0;
        U32 stride =
            vformat_stride[p->vertex_format];  // in units of sizeof(F32)
        ...
        while (pos < p->num_vertices) {
            ...
            glDrawElements(GL_TRIANGLES, (vert_count >> 2) * 6,
...
static void do_screen_quad(gswf_recti* s, F32* tc, const int* vvars,
...
static void clear_with_rect(gswf_recti* region, rrbool clear_depth,
    ...
    {
        // coordinate system doesn't match, so just draw whole screen, rely on
        // scissor to clip it properly
...
static void gdraw_DriverBlurPass(GDrawRenderState* r, int taps, F32* data,
                                 gswf_recti* s, F32* tc, F32 height_max,
                                 F32* clamp, GDrawStats* gstats) {
...
static void set_clamp_constant(GLint constant, GDrawTexture* tex) {
    ...
    // at the border we also have to convert from GDraw coords to GL coords
    // here.
    glUniform4f(constant, (s->x0 - 0.5f) / gdraw->frametex_width,
...
static void gdraw_Filter(GDrawRenderState* r, gswf_recti* s, float* tc,
    ...
    glUniform4f(prg->vars[0][VAR_filter_tc_off],
...
static void RADLINK gdraw_FilterQuad(GDrawRenderState* r, S32 x0, S32 y0,
                                     S32 x1, S32 y1, GDrawStats* gstats) {
    F32 tc[4];
    gswf_recti s;
    ...
    // if it's a rendertarget, it's inverted from our design because OpenGL is
    // bottom-left 0,0 and we have to compensate for scaling
    ...
    if (r->blend_mode == GDRAW_BLEND_filter) {
        switch (r->filter) {
            case GDRAW_FILTER_blur: {
                ...
                gdraw->in_blur = true;  // prevent viewport/projection munging
                                        // in start/end texture
                set_viewport();
                set_projection();
        ...
        // and do the blending in the pixel shader. we do this with
        // CopyTexSubImage, rather than trying to render-to-texture-all-along,
        // because that's a pain.
        // @TODO: propogate the rectangle down and only copy what we need, like
        // in 360

        if (r->blend_mode == GDRAW_BLEND_special) {
            blend_tex = (GDrawTexture*)get_color_rendertarget(gstats);
            glBindTexture(GL_TEXTURE_2D,
                          ((GDrawHandle*)blend_tex)->handle.tex.gl);
            if (gdraw->cur != gdraw->frame)
                glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, gdraw->tpw,
                                    gdraw->tph);
            else
                glCopyTexSubImage2D(GL_TEXTURE_2D, 0, gdraw->tx0 - gdraw->tx0p,
                                    gdraw->ty0 - gdraw->ty0p, gdraw->vx,
                                    gdraw->vy, gdraw->tw, gdraw->th);
        ...
        if (blend_tex) gdraw_FreeTexture(blend_tex, 0, gstats);
    }
}
...
void gdraw_GLx_(BeginCustomDraw)(IggyCustomDrawCallbackRegion* region,
                                 F32* matrix) {
    clear_renderstate();
...
static void make_vars(GDrawGLProgram prog, S32 vars[2][8], char** varn) {
    if (prog) {
        char** varn2 = (varn == pshader_general2_vars ? vshader_vsglihud_vars
                                                      : vshader_vsgl_vars);
        S32 k;
        for (k = 0; varn[k]; ++k)
            if (varn[k][0])
        ...
        if (vars[0][0] >= 0) assert(vars[0][0] != vars[0][1]);
    }
}

static void make_fragment_program(ProgramWithCachedVariableLocations* p,
                                  int num_strings, char** strings,
                                  char** varn) {
    S32 i;
    GLint res;
    GDrawGLProgram shad;
    ...
    if (!res) {
        ...
        ProgramWithCachedVariableLocations* basic_fprog_begin =
            &gdraw->fprog[0][0][0];
        ProgramWithCachedVariableLocations* basic_fprog_end =
            basic_fprog_begin +
            (sizeof(gdraw->fprog) / sizeof(gdraw->fprog[0][0][0]));
        if (p >= basic_fprog_begin && p < basic_fprog_end) {
            // for basic rendering shaders, we have three versions corresponding
            // to the three vertex formats we support.
...
static void make_vertex_program(GLuint* vprog, int num_strings,
                                char** strings) {
    GLint res;
    GDrawGLProgram shad;
    opengl_check();
...
static void bind_sampler(ProgramWithCachedVariableLocations* prog, int varidx,
                         int sampleridx) {
    int var = prog->vars[0][varidx];
    if (var >= 0) glUniform1i(var, sampleridx);
}

static void make_vertex_programs(void) {
    int type;
    for (type = 0; type < GDRAW_vformat__basic_count; type++)
        make_vertex_program(&gdraw->vert[type], NUMFRAGMENTS_vshader_vsgl,
                            vshader_vsgl(type));
    type = GDRAW_vformat_ihud1;
    make_vertex_program(&gdraw->vert[type], NUMFRAGMENTS_vshader_vsglihud,
                        vshader_vsglihud());
}
...
static void lazy_shader(ProgramWithCachedVariableLocations* ptr) {
    ...
    if (ptr >= basic_fprog_begin && ptr < basic_fprog_end) {
        ...
        make_fragment_program(ptr, NUMFRAGMENTS_pshader_basic,
                              pshader_basic_arr[n], pshader_basic_vars);
        bind_sampler(ptr, VAR_tex0, 0);
        bind_sampler(ptr, VAR_tex1, AATEX_SAMPLER);
        return;
    }

    if (ptr >= &gdraw->exceptional_blend[0] &&
        ptr < &gdraw->exceptional_blend[GDRAW_BLENDSPECIAL__count]) {
        S32 n = (S32)(ptr - gdraw->exceptional_blend);
        make_fragment_program(ptr, NUMFRAGMENTS_pshader_exceptional_blend,
                              pshader_exceptional_blend_arr[n],
                              pshader_exceptional_blend_vars);
        bind_sampler(ptr, VAR_tex0, 0);
    ...
    if (ptr >= &gdraw->filter_prog[0][0] && ptr <= &gdraw->filter_prog[1][15]) {
        ...
        make_fragment_program(ptr, NUMFRAGMENTS_pshader_filter,
                              pshader_filter_arr[n], pshader_filter_vars);
    ...
    if (ptr >= &gdraw->blur_prog[0] && ptr <= &gdraw->blur_prog[MAX_TAPS]) {
        ...
        make_fragment_program(ptr, NUMFRAGMENTS_pshader_blur,
                              pshader_blur_arr[n], pshader_blur_vars);
        bind_sampler(ptr, VAR_blur_tex0, 0);
        return;
    }

    if (ptr == &gdraw->colormatrix) {
        make_fragment_program(ptr, NUMFRAGMENTS_pshader_color_matrix,
                              pshader_color_matrix_arr[0],
                              pshader_color_matrix_vars);
        bind_sampler(ptr, VAR_colormatrix_tex0, 0);
        return;
    }

    if (ptr == &gdraw->manual_clear) {
        make_fragment_program(ptr, NUMFRAGMENTS_pshader_manual_clear,
                              pshader_manual_clear_arr[0],
                              pshader_manual_clear_vars);
        return;
    }

    if (ptr == &gdraw->ihud[0]) {
        make_fragment_program(ptr, NUMFRAGMENTS_pshader_general2,
...
static GDrawResourceLimit gdraw_limits[GDRAW_GLx_(RESOURCE__count)] = {
    ...
    16 * 1024 * 1024,  // GDRAW_GLx_RESOURCE_rendertarget
    500,
    20 * 1024 * 1024,  // GDRAW_GLx_RESOURCE_texture
    1000,
    2 * 1024 * 1024,  // GDRAW_GLx_RESOURCE_vertexbuffer
};
...
int gdraw_GLx_(SetResourceLimits)(gdraw_resourcetype type, S32 num_handles,
                                  S32 num_bytes) {
    GDrawStats stats = {0};

    if (type == GDRAW_GLx_(RESOURCE_rendertarget))  // RT count is small and
                                                    // space is preallocated
        num_handles = MAX_RENDER_STACK_DEPTH + 1;

    assert(type >= GDRAW_GLx_(RESOURCE_rendertarget) &&
           type < GDRAW_GLx_(RESOURCE__count));
    ...
    switch (type) {
        ...
        case GDRAW_GLx_(RESOURCE_texture):
            ...
            gdraw->texturecache =
                make_handle_cache(GDRAW_GLx_(RESOURCE_texture));
        ...
        case GDRAW_GLx_(RESOURCE_vertexbuffer):
            ...
            gdraw->vbufcache =
                make_handle_cache(GDRAW_GLx_(RESOURCE_vertexbuffer));
...
GDrawTexture* RADLINK gdraw_GLx_(MakeTextureFromResource)(
    ...
    // disable mipmaps if chain is incomplete and GL_TEXTURE_MAX_LEVEL is
    // unsupported
...
static GDrawFunctions* create_context(S32 w, S32 h) {
    ...
    gdraw_HandleCacheInit(
        &gdraw->rendertargets,
        gdraw_limits[GDRAW_GLx_(RESOURCE_rendertarget)].num_handles,
        gdraw_limits[GDRAW_GLx_(RESOURCE_rendertarget)].num_bytes);
    ...
    gdraw_funcs.MakeTextureFromResource =
        (gdraw_make_texture_from_resource*)gdraw_GLx_(MakeTextureFromResource);
    gdraw_funcs.FreeTextureFromResource =
        gdraw_GLx_(DestroyTextureFromResource);
...
void gdraw_GLx_(DestroyContext)(void) {
    if (gdraw) {
        ...
        if (gdraw->aa_tex) glDeleteTextures(1, &gdraw->aa_tex);

        if (gdraw->quad_ib) glDeleteBuffers(1, &gdraw->quad_ib);
    }
